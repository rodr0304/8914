...
// Add mousedown event listener on window to handle clicks outside the menu
  ...
  setFocusToMenuitem(newMenuitem) {
    // Added roving tabindex logic
    // The focused item gets tabindex="0"
    ...
    // Move keyboard focus to the new menu item
    newMenuitem.focus();
  }
  ...
  openPopup() {
    ...
    // Keep aria-expanded updated when menu opens
    this.buttonNode.setAttribute('aria-expanded', 'true');
  }

  closePopup() {
    if (this.isOpen()) {
      // Keep aria-expanded updated when menu closes
